...
const REPO_DIR = process.env.REPO_DIR ?? "/home/claude/dosmentes/dosmentes-front";
...
function parseStatusField(content, field) {
...
function isProcessActive(pattern) {
    try {
...
function readTimeline(limit = 50) {
...
function lastEventMatching(timeline, pred) {
    return timeline.find(pred) ?? null;
}
function hasActiveError(timeline, estado, bloqueos) {
    if (/^(blocked|error)/i.test(estado))
        return true;
    const b = bloqueos.trim();
    if (b && !/^ninguno/i.test(b))
        return true;
    const errEv = timeline.find((e) => e.event === "supervisor_error" ||
        e.event === "supervisor_blocked" ||
        e.event === "opus_error");
    if (!errEv)
        return false;
    const errTime = new Date(errEv.ts).getTime();
    if (Date.now() - errTime > 30 * 60 * 1000)
        return false;
    const recoveryEv = timeline.find((e) => new Date(e.ts).getTime() > errTime &&
...
function deriveAgentState(timeline, estado, bloqueos, hasNextTask, lockAgeMin, opusActive) {
    if (opusActive || (lockAgeMin !== null && lockAgeMin < 90)) {
        return { state: "running", label: "Opus implementando" };
    }
    const gitStart = lastEventMatching(timeline, (e) => e.event.includes("supervisor_git_start"));
    const gitEnd = lastEventMatching(timeline, (e) => e.event === "supervisor_end" ||
        e.event.includes("supervisor_pr_merged") ||
...
function lastEventTsInLog(agent, events) {
    const content = readSafe(PATHS.log);
    const lines = content.trim().split("\n").filter(Boolean);
...
function cronStatus(logFile, agent, startEvents, scheduleMin) {
...
function lastOpusOutputTail(lines = 5) {
    try {
        const files = fs
...
function countOpenPrs() {
    try {
        const out = execSync(`gh pr list --repo ${GH_REPO} --state open --json number --jq 'length'`, { encoding: "utf8", timeout: 15000 });
...
function extractUc(text) {
    const m = text.match(/UC-DM-(?:S[0-9]+|INFRA)-[0-9]+/);
    return m?.[0] ?? null;
}
function deriveHealth(cronSup, cronOpus, agentState) {
    if (agentState === "error")
        return "error";
    if (!cronSup && !cronOpus)
        return "error";
...
export function collect() {
    ...
    const agentDerived = deriveAgentState(timeline, estado, bloqueos, hasNextTask, lockAgeMin, opusActive);
    const epicMatch = rama.match(/^epic\/[^/]+/) ?? tarea.match(/epic\/[^\s`]+/);
    const activeEpicBranch = epicMatch?.[0] ?? null;
    const currentUc = extractUc(tarea) ?? extractUc(nextTaskContent) ?? extractUc(rama);
    ...
    const cronSupervisor = cronStatus(PATHS.cronSupervisor, "supervisor", ["supervisor_start"], 10);
    const cronOpus = cronStatus(PATHS.cronOpus, "opus-worker", ["opus_start", "opus_done", "opus_tick"], 5);
    if (opusActive) {
    ...
    const snapshot = {
        ...
        timeline,
        health: deriveHealth(cronSupervisor.healthy, cronOpus.healthy, agentDerived.state),
    };
