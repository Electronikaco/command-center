...
function shortUc(uc: string | null): string {
...
function moodFromState(
  state: AgentVisualState,
  badge: ClaudeBuddy["statusBadge"],
): { mood: BuddyMood; label: string } {
  switch (state) {
    case "running":
...
export function buildBuddy(
  ...
  const stats: BuddyStat[] = [
    ...
    },
    {
      key: "PATIENCE",
      value: clamp(
        agentState === "waiting" ? 88 : lockAgeMin && lockAgeMin > 30 ? 40 : 70,
      ),
    },
    {
      key: "CHAOS",
    ...
    },
    {
      key: "SNARK",
      value: clamp(
        agentState === "error" ? 70 : agentState === "idle" ? 45 : 30,
      ),
    },
  ];
