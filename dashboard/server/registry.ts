...
import type { RegistryProject } from "../shared/types.js";
...
function parseRegistryYaml(content: string): RegistryProject[] {
  const projects: RegistryProject[] = [];
  let current: RegistryProject | null = null;
  let section: string[] = [];

  const flushSection = () => {
    if (!current || section.length === 0) return;
    const [head, ...rest] = section;
    const key = head.replace(/:$/, "");
    const obj: Record<string, string> = {};
    for (const line of rest) {
      const m = line.match(/^\s{4,}(\w+):\s*(.+)$/);
      if (m) obj[m[1]] = String(parseScalar(m[2]));
    }
    if (key === "progress")
      current.progress = obj as unknown as RegistryProject["progress"];
    else if (key === "health") current.health = obj as RegistryProject["health"];
    else if (key === "labels") current.labels = obj;
    else if (key === "links") current.links = obj;
    section = [];
  };

  for (const line of content.split("\n")) {
    if (line.match(/^projects:\s*$/)) continue;
...
export function loadRegistry(
  registryPath = process.env.PROJECTS_REGISTRY ?? DEFAULT_REGISTRY,
): RegistryProject[] {
